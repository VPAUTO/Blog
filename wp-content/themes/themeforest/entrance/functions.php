	...
	function g7_image($w, $h, $link = true, $overlay = true) {
		...
		if ($link) {
			...
			switch (get_post_format()) {
				case 'image':
					$icon       = 'fa-picture-o';
					$link_open  = '<a href="'.$img_url.'" rel="prettyPhoto">';
					$link_close = '</a>';
					break;
				case 'video':
					$icon = 'fa-play-circle';
...
if (!function_exists('g7_image_sizes')) {
	function g7_image_sizes($type = 'thumb') {
		$sizes = array(
			...
			'featured1'   => array(720, 554),
			'featured2'   => array(480, 369),
 ...
 */
if (!function_exists('get_the_first_category_ID')) {
	function get_the_first_category_ID() {
 ...
 */
if (!function_exists('get_the_first_category')) {
